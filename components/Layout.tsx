...
const Layout: React.FC<LayoutProps> = ({ children, activePage, onNavigate, user, onTriggerPremium }) => {
  ...
  const NavItem = ({ page, icon: Icon, label, highlight = false }: { page: string, icon: any, label: string, highlight?: boolean }) => (
    <button
      ...
      className={`relative flex items-center ${isCollapsed ? 'justify-center' : 'space-x-3'} w-full p-3 rounded-xl transition-all duration-300 group overflow-hidden ${activePage === page
    ...
    <div className="flex h-screen bg-[#020202] text-slate-100 overflow-hidden font-sans selection:bg-brand-purple selection:text-white">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative z-10">
        ...
        <main className="flex-1 overflow-y-auto scroll-smooth">
          <div className="p-3 md:p-10 max-w-7xl mx-auto pb-24 md:pb-10 min-h-[calc(100vh-80px)]">
            {children}
          </div>

          <footer className="p-6 border-t border-white/5 text-center text-slate-600 text-xs">
            <div className="flex justify-center gap-6 mb-2">
              <button onClick={() => onNavigate('terms')} className="hover:text-slate-400 transition-colors">Termos de Uso</button>
              <button onClick={() => onNavigate('privacy')} className="hover:text-slate-400 transition-colors">Política de Privacidade</button>
              <a href="mailto:support@carryme.gg" className="hover:text-slate-400 transition-colors">Suporte</a>
            </div>
            <p>&copy; {new Date().getFullYear()} CarryMe.gg. Todos os direitos reservados.</p>
          </footer>
        </main>
      </div>
    </div>
