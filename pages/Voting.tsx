...
const Voting: React.FC<VotingProps> = ({ onComplete }) => {
  // No cases of mock matches anymore. In a real app we'd fetch actual pending votes.
  const match: Match | undefined = undefined;
  ...
  if (!match) return (
    <div className="p-20 text-center bg-slate-900/50 border border-slate-800 border-dashed rounded-3xl">
      <Check size={48} className="mx-auto text-green-500 mb-4 opacity-50" />
      <h2 className="text-xl font-bold text-white mb-2">Sem Pendências</h2>
      <p className="text-slate-400">Você já avaliou todos os seus aliados recentes.</p>
      <button onClick={onComplete} className="mt-8 px-8 py-3 bg-slate-800 text-white font-bold rounded-xl border border-slate-700">Voltar ao Painel</button>
    </div>
  );
