...
import { ItemType, Player, StoreItem } from '../types';
import { Coins, Lock, Check, Plus, Filter } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="flex space-x-2 border-b border-slate-800 pb-1 overflow-x-auto">
        <button 
          ...
          className={`px-6 py-3 font-bold border-b-2 transition-colors whitespace-nowrap ${activeTab === ItemType.BORDER ? 'border-blue-500 text-white' : 'border-transparent text-slate-500 hover:text-slate-300'}`}
        >
          Bordas
        </button>
        <button 
          onClick={() => setActiveTab(ItemType.NAME_COLOR)}
          className={`px-6 py-3 font-bold border-b-2 transition-colors whitespace-nowrap ${activeTab === ItemType.NAME_COLOR ? 'border-blue-500 text-white' : 'border-transparent text-slate-500 hover:text-slate-300'}`}
        >
          Cores de Nome
        </button>
        <button 
          onClick={() => setActiveTab(ItemType.BANNER)}
          className={`px-6 py-3 font-bold border-b-2 transition-colors whitespace-nowrap ${activeTab === ItemType.BANNER ? 'border-blue-500 text-white' : 'border-transparent text-slate-500 hover:text-slate-300'}`}
        >
          Banners
        </button>
      </div>
            ...
            return (
              <div key={item.id} className={`bg-slate-900 border rounded-2xl p-6 flex flex-col transition-all duration-300 hover:-translate-y-1 ${rarityClass.split(' ')[1]}`}>
                ...
                <div className="h-32 bg-slate-950 rounded-xl mb-6 flex items-center justify-center relative overflow-hidden">
                  ...
                  {item.type === ItemType.BANNER && (
                    <div className={`absolute inset-0 ${item.value}`}></div>
                  )}
                </div>
